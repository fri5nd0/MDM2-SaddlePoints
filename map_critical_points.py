...
print("\n=== Closest Saddle Points to Trajectory ===")
...
# === Highlight closest saddle points ===
...
ax.set_title("3D Landscape: Trajectory and Closest Saddle Points", fontsize=14)
...
# === 2D localized plots (adaptive axes) ===
for i, row in closest_saddles.iterrows():
    saddle_idx = int(row["saddle_index"])
    closest_step = int(row["closest_traj_step"])
    saddle_proj = projected_saddles[saddle_idx]

    # Local window of trajectory
    window = 20
    lo = max(0, closest_step - window)
    hi = min(len(projected_trajectory), closest_step + window)
    local_traj = projected_trajectory[lo:hi]

    # --- Adaptive axis selection: find two axes with largest local variance ---
    local_var = np.var(local_traj, axis=0)
    local_top2 = np.argsort(local_var)[-2:][::-1]

    plt.figure(figsize=(6, 5))
    plt.plot(local_traj[:, local_top2[0]], local_traj[:, local_top2[1]], color="orange", lw=2)
    plt.scatter(local_traj[0, local_top2[0]], local_traj[0, local_top2[1]], color="black", s=60, label="Segment start")
    plt.scatter(local_traj[-1, local_top2[0]], local_traj[-1, local_top2[1]], color="lime", s=60, label="Segment end")
    plt.scatter(saddle_proj[local_top2[0]], saddle_proj[local_top2[1]], color="red", s=150, marker="D", label="Saddle")

    plt.title(f"Saddle {saddle_idx}: Local Trajectory Behavior", fontsize=13)
    plt.xlabel(f"PCA Axis {top3_idx[local_top2[0]]}")
    plt.ylabel(f"PCA Axis {top3_idx[local_top2[1]]}")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.show(block=True)
